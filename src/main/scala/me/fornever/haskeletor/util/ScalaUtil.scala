...
object ScalaUtil:
  ...
  implicit class RichBoolean(val b: Boolean) extends AnyVal:
    final def option[A](a: => A): Option[A] = if b then Option(a) else None

    final def optionNot[A](a: => A): Option[A] = if b then None else Option(a)
