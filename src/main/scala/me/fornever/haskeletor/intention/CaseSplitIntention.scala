...
object CaseSplitIntention:
  ...
  import me.fornever.haskeletor.intention.CaseSplitIntention.CaseSplitKind.*
  ...
  private def subTopLevelSplitting(psiElement: PsiElement): Option[CaseSplitKind] =
    if psiElement.getNode.getElementType == HaskellTypes.HS_UNDERSCORE then
  ...
  private def topLevelCaseSplitting(psiElement: PsiElement): Option[CaseSplitKind] =
    if psiElement.getNode.getElementType == HaskellTypes.HS_UNDERSCORE then
      if Option(TreeUtil.findSiblingBackward(psiElement.getNode, HaskellTypes.HS_EQUAL)).isEmpty &&
        Option(TreeUtil.findSibling(psiElement.getNode, HaskellTypes.HS_EQUAL)).isDefined then
      ...
      HaskellPsiUtil.findQName(psiElement).flatMap(ne =>
        if Option(PsiTreeUtil.findSiblingBackward(ne, HaskellTypes.HS_EQUAL, null)).isEmpty &&
          (Option(PsiTreeUtil.findSiblingForward(ne, HaskellTypes.HS_EQUAL, null)).isDefined) then {
  ...
  private def doTopLevelSplitCase(topLevel: TopLevel, project: Project, constrs: Seq[HaskellCompositeElement], currentDeclarationLine: HaskellTopDeclaration): Unit =
    constrs.foreach { constr =>
      val newLine = if currentDeclarationLine.getNode.getChildren(TokenSet.create(HaskellTypes.HS_NEWLINE)).nonEmpty then
      ...
      if HaskellPsiUtil.findNamedElements(constr).size > 1 then
  ...
  private def createPattern(project: Project, constr: HaskellCompositeElement, currentLineElements: Seq[HaskellNamedElement]): PsiElement =
    def defaultPatternNamedElements(elements: Seq[HaskellQName]) =
      if elements.map(_.getIdentifierElement).exists(_.isInstanceOf[HaskellConsym]) then
