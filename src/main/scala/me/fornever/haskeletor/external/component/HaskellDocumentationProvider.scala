...
import me.fornever.haskeletor.util.{HaskellEditorUtil, HaskellProjectUtil, HtmlElement}
...
class HaskellDocumentationProvider extends AbstractDocumentationProvider:
  ...
  override def getQuickNavigateInfo(element: PsiElement, originalElement: PsiElement): String =
    ...
    if project.exists(p => !StackProjectManager.isInitializing(p)) then
      (Option(element.getContainingFile), Option(originalElement)) match
        case (Some(file), Some(oe)) =>
          ...
          val typeSignature = if isSourceFile then
          ...
          (moduleName, typeSignature) match
            ...
            case (Some(mn), None) => s"""$DoubleNbsp $mn $DoubleNbsp -- No type info available""" + (if isSourceFile then " (at this moment)" else "")
    ...
    if project.exists(p => !StackProjectManager.isInitializing(p)) then
      Option(element) match
        case Some(e) =>
          ...
          if e.isInstanceOf[PsiFile] then
