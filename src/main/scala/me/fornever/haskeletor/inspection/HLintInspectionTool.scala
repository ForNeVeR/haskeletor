...
class HLintInspectionTool extends LocalInspectionTool:
    ...
    HaskellFileUtil.findDocument(psiFile) match
      case Some(document) if HaskellProjectUtil.isSourceFile(psiFile) =>
        ...
        yield
          ...
          val quickFix = if hlintInfo.refactorings == "[]" then
          ...
          quickFix match
            case Some(qf) =>
              val endElement = if qf.getEndElement == null then qf.getStartElement else qf.getEndElement
  ...
  private def findOffsets(document: Document, virtualFile: VirtualFile, psiFile: PsiFile, pos: SrcSpan, newText: Option[String]) =
    ...
    yield
      val replaceEndOffset2 = if newText.exists(_.isEmpty) && replaceEndOffset < psiFile.getTextLength - 1 then
        replaceEndOffset + 1
      else
        replaceEndOffset
