...
class HaskellCompletionContributor extends CompletionContributor:
  ...
  private val provider: CompletionProvider[CompletionParameters] = new CompletionProvider[CompletionParameters]:
    def addCompletions(parameters: CompletionParameters, context: ProcessingContext, originalResultSet: CompletionResultSet): Unit =
      ...
      if StackProjectManager.isInitializing(project) then
      ...
      else if !isConsoleFile && (!HaskellProjectUtil.isSourceFile(psiFile) || Option(parameters.getOriginalPosition).map(_.getNode.getElementType).exists(t => HaskellParserDefinition.Literals.contains(t) || HaskellParserDefinition.Comments.contains(t))) then
        ()
      else

        ProgressManager.checkCanceled()

        val positionElement = if isConsoleFile then
          None
        else
          ...
          if Option(parameters.getOriginalPosition).exists(_.getNode.getElementType == HS_NEWLINE) then
        ...
        val prefixText = (
          ...
          for
            ...
            start = if qne.getText.startsWith("(") || qne.getText.startsWith("`") then qne.getTextRange.getStartOffset + 1 else qne.getTextRange.getStartOffset
            end = parameters.getOffset
          yield psiFile.getText.substring(start, end)
        }).orElse(
          for
          ...
          yield t
        ).flatMap(pt => if pt.trim.isEmpty then None else Some(pt))
        ...
        positionElement match
          ...
          case Some(e) if isImportModuleDeclarationInProgress(e) =>
            ...
            if e.getParent.getNode.getElementType != HS_QUALIFIER then
          ...
          case Some(position) if HaskellPsiUtil.findExpression(position).isDefined & position.getText.startsWith("_") =>
            ...
            HaskellAnnotator.getHighlightingTooltipHtml(project, offset, editor) match
              case Some(message) =>
                ...
                if typedHoleSuggestionsWithHeader.nonEmpty then
        ...
        HaskellPsiUtil.findModIdElement(ce) match
          ...
          case _ => findQualifiedNamedElementToComplete(ce) match
            case Some(qualifiedNameElement) =>
              ...
              val endOffset = if qualifiedNameElement.getText.endsWith("`") || qualifiedNameElement.getText.endsWith(")") then
        ...
        e =>
          d match {
            ...
            case d: HaskellDeclarationElementImpl if d.isInstanceOf[HaskellTypeSignature] =>
              if !d.getIdentifierElements.headOption.map(_.getName).exists(n => expressionElements.toSeq.map(_.getName).contains(n)) then {
  ...
  private def createLookupElement(moduleIdentifier: ModuleIdentifier, addParens: Boolean = false): LookupElementBuilder =
    val completion = if moduleIdentifier.operator && addParens then
